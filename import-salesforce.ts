...
export const handler: Handler = async (event) => {
  ...
  const { data: integ } = await supabase
    ...
    .eq('org_id', org_id).eq('provider', 'salesforce').maybeSingle()
  const access = (integ as any)?.access
  if (!access?.access_token || !access?.instance_url) return { statusCode: 400, body: 'missing salesforce integration' }

  const soql = `SELECT Id, FirstName, LastName, Email, Title, Account.Name FROM Contact WHERE Email != null LIMIT ${limit}`
  const url = `${access.instance_url}/services/data/v58.0/query?q=${encodeURIComponent(soql)}`
  const resp = await fetch(url, { headers: { Authorization: `Bearer ${access.access_token}` } })
  const json: any = await resp.json()
  if (!resp.ok) return { statusCode: resp.status, body: JSON.stringify(json) }

  const rows = (json.records || []).map((c: any) => ({
  ...
  const positions = rows.filter((r:any)=>r.company || r.title).map((r:any) => ({
    champion_id: idByEmail.get(r.email),
    company: r.company,
    title: r.title,
    is_current: true,
  }))
